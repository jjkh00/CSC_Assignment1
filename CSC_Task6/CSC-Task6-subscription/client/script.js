...
function confirmPriceChange() {
  changeLoadingStatePlanChange(true);
  const params = new URLSearchParams(document.location.search.substring(1));
  const subscriptionId = params.get('subscriptionId');
...
function cancelSubscription() {
  changeLoadingStatePrices(true);
  changeLoadingStateCancelSub(true);
  const params = new URLSearchParams(document.location.search.substring(1));
  const subscriptionId = params.get('subscriptionId');
...
// Show a loader on plan changes
function changeLoadingStatePlanChange(isLoading) {
  if (isLoading) {
    document.querySelector('#loader').classList.remove('hidden');
  } else{
    document.querySelector('#loader').classList.add('hidden');
...
function changeLoadingStateCancelSub(isLoading) {
  if (isLoading) {
    document.querySelector('#loader_subscription').classList.remove('hidden');
  } else{
    document.querySelector('#loader_subscription').classList.add('hidden');
  }
}
